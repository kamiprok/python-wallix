...
def search():
    try:
        ...
        swloro = input('Input issue ID number(SWLOROTST): ')
        issue = jira.issue(f'SWLOROTST-{swloro}')
        ...
        print(f'Issue SWLOROTST-{swloro} does not exist!')
            ...
            if message.receivedtime.timestamp() > day_ago:
                ...
                if message.Unread == True:
                    ...
                    if proceed.lower() == 'y':
                        ...
                        last_issue = jira.search_issues('project=SWLOROTST')[0]
                        # print(last_issue)
                        ...
                        try:
                            if create_link.lower() == 'y':
                                parent_number = input('Input issue ID number(SWLOROTST): ')
                                print('Found existing ticket under number SWLOROTST-'+parent_number)
                                new_issue = jira.search_issues('project=SWLOROTST')[0]
                                # print(new_issue)
                                parent_issue = ('SWLOROTST-' + parent_number)
                                # print(parent_issue)
...
def last_issue():
    ...
    # last_issue_full = jira.search_issues('project=SWLOROTST')[:1]
    # print('Last issue full info:', last_issue_full)
    # all_issues = jira.search_issues('project=SWLOROTST')[:11]
...
def listen_for_issues():
    '''
    # shows current time and converts it to format that is recognizable by JIRA
    now = datetime.datetime.utcnow()
    hour_ago = now - timedelta(hours=-1)
    now = now.strftime("%Y-%m-%dT%H:%M:%S.125+0100")
    hour_ago = hour_ago.strftime("%Y-%m-%dT%H:%M:%S.125+0100")
    print(now)
    print(hour_ago)

    # module to link new issue to an existing one
    create_link = input('Do you want to link this ticket to an existing one? [Y/N]: ')
    try:
        if create_link.lower() == 'y':
            parent_number = input('Input issue ID number(SWLOROTST): ')
    ...
    for message in messages:
        if message.receivedtime.timestamp() > day_ago:
            if message.Unread == True:
                print('Status: ', message.Unread)
                print(message.receivedtime)
                print(message.subject)

    issues = jira.search_issues('project=SWLOROTST', 0, 10)
    # for issue in issues:
        # print(issue)
    ...
    while True:
        try:
            for message in messages:
                if message.receivedtime.timestamp() > day_ago:
                    ...
                    if message.Unread == True:
                        ...
                        # print("Didn't find any new emails. Going to sleep for 5 seconds.")
                        time.sleep(5)
            ...
            # print('No more unread emails. Going to sleep for 5 seconds.')
            time.sleep(5)
...
def create_single_link():
    try:
        ...
        new_issue = input('Input issue ID number you want to link(SWLOROSTS-): ')
        parent_number = input('Input issue ID number you want it to be linked to(SWLOROTST-): ')
        print('Found existing ticket under number SWLOROTST-' + parent_number)
        new_issue = f'SWLOROTST-{new_issue}'
        parent_issue = ('SWLOROTST-' + parent_number)
        print('Linking', new_issue, 'to', parent_issue)
        jira.create_issue_link('relates to', new_issue, parent_issue, None)
        print(new_issue, 'issue linked to', parent_issue)
